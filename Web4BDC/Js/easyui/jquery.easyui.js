...
cc.push("<li>");
cc.push("<div id=\"" + item.domId + "\" node-id=\"" + item.id + "\" class=\"tree-node\">");
...
var col=cols[j];
var attr = "";
if (col.dynamicHead!=undefined&&!col.dynamicHead) {
    continue;
}
if(col.rowspan){
...
if (!col.hidden && !col.checkbox && !col.auto && !col.fixed && (col.dynamicHead==undefined||col.dynamicHead)) {
return true;
}
};
};
...
if (col && (col.dynamicHead == undefined || col.dynamicHead)) {
var _75f=_75c[_75e];
...
cc.push(" name=\""+_75e+"\" value=\""+(_75f!=undefined?_75f:"")+"\">");
    ...
    if (col.formatter) {
        var field = col.formatter(_75f, _75c, _75b);
        if ($(field).attr("display") && $(field).attr("display").toUpperCase() == "TRUE")
        {
            cc.push(field);
        }
        else if (!$(field).attr("display")) {
            cc.push(field);
        }
}else{
cc.push(_75f);
